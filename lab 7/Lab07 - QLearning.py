...
class Agent():
    ...
    def __init__(self,seed,state_dims,actions,learning_rate,discount_factor,eps_greedy,decay):
        ...
        qtable = np.zeros((self.state_dims[0], self.state_dims[1])) #Initializing Q-Table
        for i in range(self.state_dims[0]):
            for j in range(self.state_dims[1]):
                qtable[i][j] = self.prng.random()
    ...
    def step(self, env, learn=True):
        ...
        state_dimentions = env.get_state_dimensions()
        print("dim", state_dimentions)
        ...
        is_terminal_state = env.is_terminal_state()
        print("is_terminal_state", is_terminal_state)

        self.prng.random(0,3)
        reward, new_state = env.perform_action(self.prng.random(0,3))
        print("reward", reward)
        print("new_state", new_state)

        #valid = env.get_valid(new_state)
        #print("valid", valid)

        print("learn", learn)

        print(self.prng.random(), self.eps_greedy)

        #while(not env.is_terminal_state()):
        #    reward, new_state = env.perform_action(0)
