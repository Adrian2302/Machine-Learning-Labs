...
### Default values
DEFAULT_ALPHA_LR = 0.7
DEFAULT_GAMMA_DISCOUNT = 0.9
DEFAULT_EPS_GREEDY = 0.9
DEFAULT_ETA_DECAY = 0.000001
...
class Agent():
    ...
    def __init__(self,seed,state_dims,actions,learning_rate,discount_factor,eps_greedy,decay):
        ...
        self.decay = decay
        self.lista_estados = []
        self.qtable = np.zeros((math.prod(self.state_dims),len(self.actions)))
    
    # Performs a complete simulation by the agent
    def simulation(self, env):
        for i in range(100000):
            env.reset()
            while(env.is_terminal_state() != True):
                self.step(env)
    ...
    def step(self, env, learn=True):
        ...
        if current_state not in self.lista_estados:
            self.lista_estados.append(current_state)
        state = self.lista_estados.index(current_state)

        if(self.prng.random() <= self.eps_greedy and learn):
            #print("Estoy aprendiendo")

            rewards = []
            for i in range(4):
                rewards.append(self.qtable[state][i])

            current_action = self.prng.randint(0, 3)

            reward, new_state = env.perform_action(current_action)

            if(new_state in self.lista_estados):
                new_state_index = self.lista_estados.index(new_state)
            else:
                self.lista_estados.append(new_state)
                new_state_index = self.lista_estados.index(new_state)

            current_state_index = self.lista_estados.index(current_state)
            
            self.qtable[current_state_index][current_action] = self.learning_rate*(reward+self.discount_factor*np.max(self.qtable[new_state_index][:]-self.qtable[current_state_index][current_action]))
            ...
            index = rewards.index(max_reward)
            reward, new_state = env.perform_action(index)
...
NOTA: El botón de skip no me sirve, entonces metí 100 000 iteraciones en el método de simulation.

Patrones interesantes que noté:
    - Tal vez que por lo general el agente toma el camino más corto hacia las llaves o hacia el final,
      quiero decir como que no hace vueltas innecesarias por rutas donde no tengan sentido.

Mis semillas de mapas interesantes:
    - Semilla 69 con llave, learning rate = 0.7, gamma = 0.9, epsylon greedy = 0.9, decay = 0.000001 y 600 000 simulaciones
    - Semilla 2302 con llave y tesoros, learning rate = 0.7, gamma = 0.9, epsylon greedy = 0.9, decay = 0.000001 y 500 000 simulaciones

MAPA SEMILLA 21 (Con llave): 
Estoy utilizando un learning rate = 0.7, gamma = 0.9, epsylon greedy = 0.9, decay = 0.000001 y 
con 500 000 simulaciones para lograr completarlo.

MAPA SEMILLA 17 (Solo):
Estoy utilizando un learning rate = 0.7, gamma = 0.9, epsylon greedy = 0.9, decay = 0.000001 y 
con 500 000 simulaciones para lograr completarlo.

a. También debe probar algunos casos aplicando tesoros. ¿Qué cambia en el comportamiento?

    R/ Ya mi agente no va directamente hacia la salida, sino que se toma la molestia de agarrar
       todas las llaves antes de terminar. Con la semilla 2302 es un ejemplo de cuando utilicé 
       llave y tesoros, mi agente hizo una vuelta al laberinto para poder agarrar todos los 
       tesoros antes de acabar.
